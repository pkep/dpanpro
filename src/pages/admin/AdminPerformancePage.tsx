  ...
  const { data: kpis, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const avgResponseTime = validResponseTimes.length > 0 
        ? validResponseTimes.reduce((a, b) => a + b, 0) / validResponseTimes.length : null;
      ...
      const avgArrivalTime = validArrivalTimes.length > 0 
        ? validArrivalTimes.reduce((a, b) => a + b, 0) / validArrivalTimes.length : null;
      ...
      const { data: attempts, error: attemptsError } = await supabase
        .from('dispatch_attempts').select('status');
      ...
      const { data: completedInterventions, error: completedError } = await supabase
        .from('interventions').select('id').eq('status', 'completed');
      if (completedError) throw completedError;

      const { data: quoteModifications, error: quotesError } = await supabase
        .from('quote_modifications').select('intervention_id');
      ...
      return { avgResponseTime, avgArrivalTime, avgRating, acceptanceRate, avgRevenuePerTech, firstPassRate };
    },
  });

  const getStatusColor = (value: number | null, target: number, isLower: boolean = true) => {
    if (value === null) return 'text-muted-foreground';
    if (isLower) return value <= target ? 'text-green-600' : 'text-orange-600';
    ...
    <AdminLayout title="Dashboard Performance" subtitle="KPIs et analyses">
      <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="px-3 sm:px-6 pb-3 sm:pb-6">
              {isLoading ? (
                ...
                <>
                  <div className="text-lg sm:text-xl font-bold">{formatDuration(kpis?.avgResponseTime || null)}</div>
          ...
          <Card>
            ...
            <CardContent className="px-3 sm:px-6 pb-3 sm:pb-6">
              {isLoading ? (
                ...
                <>
                  <div className="text-lg sm:text-xl font-bold">{formatDuration(kpis?.avgArrivalTime || null)}</div>
          ...
          <Card>
            ...
            <CardContent className="px-3 sm:px-6 pb-3 sm:pb-6">
              {isLoading ? (
                ...
                <>
                  <div className="text-lg sm:text-xl font-bold">{kpis?.firstPassRate ? `${kpis.firstPassRate.toFixed(0)}%` : '--%'}</div>
          ...
          <Card>
            ...
            <CardContent className="px-3 sm:px-6 pb-3 sm:pb-6">
              {isLoading ? (
                ...
                <>
                  <div className="text-lg sm:text-xl font-bold">{kpis?.acceptanceRate ? `${kpis.acceptanceRate.toFixed(0)}%` : '--%'}</div>
          ...
          <Card>
            ...
            <CardContent className="px-3 sm:px-6 pb-3 sm:pb-6">
              {isLoading ? (
                ...
                <>
                  <div className="text-lg sm:text-xl font-bold">{formatCurrency(kpis?.avgRevenuePerTech || 0)}</div>
        ...
        <Tabs defaultValue="reports" className="space-y-4">
          ...
          <TabsContent value="reports"><PerformanceReportsTab /></TabsContent>
          <TabsContent value="analysis"><PerformanceAnalysisTab /></TabsContent>
          <TabsContent value="heatmap"><PerformanceHeatmapTab /></TabsContent>
          <TabsContent value="ranking"><PerformanceRankingTab /></TabsContent>
        </Tabs>
      </div>
    </AdminLayout>
  );
