...
function generateQuoteNumber(interventionId: string, date: Date): string {
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const quoteDate = intervention.quote_signed_at ? new Date(intervention.quote_signed_at) : new Date();
    const quoteNumber = generateQuoteNumber(interventionId, quoteDate);
    ...
    const quoteBase64 = dataUri.split(",")[1];
    const quoteFileName = `devis-${quoteNumber}.pdf`;

    return new Response(JSON.stringify({ success: true, quoteBase64, quoteFileName, quoteNumber }), {
      status: 200,
      headers: { "Content-Type": "application/json", ...corsHeaders },
    });
  } catch (error: any) {
